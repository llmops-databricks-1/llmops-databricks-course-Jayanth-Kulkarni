...
sp = w.service_principals.create(display_name="lakebase-sp-hf-docs")
...
project_id = "hf-docs-agent-lakebase"
w = WorkspaceClient()
...
with psycopg.connect(conn_string) as conn:
    conn.execute(f'GRANT USAGE ON SCHEMA public TO "{client_id}";')
    conn.execute(
        f'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE session_messages TO "{client_id}";'
    )
    conn.execute(
        f'GRANT USAGE, SELECT ON SEQUENCE session_messages_id_seq TO "{client_id}";'
    )
