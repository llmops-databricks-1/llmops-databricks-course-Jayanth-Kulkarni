...
# MAGIC - Vector Search MCP over HF Docs index
...
from stackoverflow_curator.mcp import create_mcp_tools
...
try:
    ...
    vs_tools = []
    logger.warning(
        f"MCP endpoint not available: {type(e).__name__}. "
        "Feature may not be enabled on this workspace."
    )
...
# MAGIC
# MAGIC The MCP tool name uses double underscores:
# MAGIC - Tool name: `mlops_dev__jayanthk__hf_docs_index`
# MAGIC - Parameter: `query` (just the search query text)

# COMMAND ----------

tool_name = f"{cfg.catalog}__{cfg.schema}__hf_docs_index"

try:
    search_result = vs_mcp_client.call_tool(
        tool_name, {"query": "how to load a pretrained model"}
    )
    ...
    logger.warning(f"MCP call_tool failed: {type(e).__name__}. Skipping.")
...
if hasattr(cfg, "genie_space_id") and cfg.genie_space_id:
    ...
    try:
        ...
        logger.warning(f"Genie MCP not available: {type(e).__name__}")
else:
...
vector_search_tool_name = f"{cfg.catalog}__{cfg.schema}__hf_docs_index"

if vector_search_tool_name in tools_dict:
    search_tool = tools_dict[vector_search_tool_name]

    result = search_tool.exec_fn(query="transformer fine-tuning with LoRA")
...
# MAGIC %md
# MAGIC ## 10. Using MCP Tools with an Agent
...
response = agent.chat("How do I use the AutoModel class to load a pretrained model?")
...
# MAGIC 5. Listing and calling MCP tools against `hf_docs_index`
...
# MAGIC
# MAGIC **Next**: Lecture 3.2b - Genie Space Integration
